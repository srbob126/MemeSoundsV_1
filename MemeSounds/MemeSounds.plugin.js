	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Api) => { try {
			...
			const sounds = [
				...
				{re: /real estate/gmi, file: "free-real-estate-By-Tuna.mp3", duration: 100},
				{re: /wop/gmi, file: "wop.mp3", duration: 200},
				{re: /hard/gmi, file: "bingbong.mp3", duration: 100}
			];
				...
				messageEvent = async ({ channelId, message, optimistic }) => {
					...
					if (!optimistic && lastMessageID != message.id) {
						...
						for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
							let audio = new Audio("https://github.com/srbob126/MemeSoundsV_1/raw/main/MemeSounds/Sounds/"+sound[1].file);
